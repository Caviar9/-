...
class CustomWebPage(QWebEnginePage):
    def userAgentForUrl(self, url):
        # 使用最新的Chrome User-Agent，模拟真实浏览器环境
        return (
...
class MiniBrowser(QMainWindow):
    def __init__(self):
        ...
        # 添加移动版抖音按钮
        mobile_button = QPushButton('📱移动版')
        mobile_button.setFixedWidth(60)
        mobile_button.clicked.connect(self.load_mobile_douyin)
        url_layout.addWidget(mobile_button)
        ...
        # 设置WebEngineProfile的全局User-Agent
        ...
        # 旧版 PyQt5 可能不提供 QWebEngineScript；改为在 loadFinished 时注入
        
        self.web_view.setPage(CustomWebPage(self.web_view))
        self.web_view.load(QUrl("https://www.douyin.com"))
        self.web_view.urlChanged.connect(self.update_url_input)
        self.web_view.loadFinished.connect(self.apply_page_dark_css)
    ...
    def apply_theme(self, mode):
        ...
        if mode == 'dark':
            ...
            palette.setColor(QPalette.Link, QColor("#3d6cff"))
            ...
            app.setStyleSheet(
                ...
                QLineEdit, QTextEdit, QPlainTextEdit { background-color: #000000; color: #ffffff; selection-background-color: #3d6cff; }
                QPushButton { background-color: #000000; color: #ffffff; border: 1px solid #333333; padding: 4px 8px; }
                QPushButton:pressed { background-color: #111111; }
                QSlider::groove:horizontal { background: #222222; height: 6px; border-radius: 3px; }
                QSlider::handle:horizontal { background: #555555; width: 12px; margin: -5px 0; border-radius: 6px; }
                QLabel { color: #ffffff; }
                """
            )
            self.update_button_styles()

    def load_mobile_douyin(self):
        # 加载移动版抖音
        mobile_url = "https://m.douyin.com"
        self.url_input.setText(mobile_url)
        self.load_url()

    def apply_page_dark_css(self, ok):
        if not ok:
            return
        # 在页面注入黑色主题 CSS 和浏览器检测绕过
        js = r"""
        (function(){
            try {
                // 绕过浏览器版本检测
                ...
                style.textContent = `
                    ...
                    [style*="background"], [class*="bg"], header, footer, nav, main, section, article, aside {
                        background-color: #000 !important;
                    }
                    a { color: #7aa2ff !important; }
                    input, textarea, select { background: #000 !important; color: #fff !important; border-color: #333 !important; }
                `;
            } catch (e) {}
        })();
        """
